...
- 首頁與收支分析統一顯示「投資淨投入」＝買進實付－賣出實收。
- 賣出採實際實收（成交金額－手續費－證交稅），不使用原始買入成本。
- 家庭／居家生活細分類重新劃界，並做一次性舊名稱安全遷移。
- 主記帳表單的細分類改為底部選擇面板，近期常用項目優先。
- 支出圓環同時顯示金額與百分比，圓環與圖例皆可點擊查看明細。
- 分析期間支援左右滑動切換。
- 放大關鍵文字與金額、統一數字字寬、降低卡片陰影。
- 保留 V1.5.5 自動股息行事曆與資料結構 V13。

V1.5.5 延續 V1.5.4 的 GitHub Pages 靜態市場資料架構，新增「自動股息行事曆」。資料結構仍為 V13，不需要重新匯入既有記帳／投資資料。

## V1.5.5：自動股息行事曆

- 每日由 GitHub Action 更新 `data/dividend-calendar.json`。
- ETF 以 TWSE ETF e添富官方配息清單同步除息日、收益分配基準日、預計發放日與每單位配息。
- 上市股票另以 TWSE OpenAPI 除權除息預告補充除息日與現金股利；若官方來源未提供發放日，App 保留「入帳日待補」。
- App 解鎖後自動把「目前持有且符合資格」的官方事件合併進本機 `dividendEvents`。
- 除息日到達後凍結 `entitledShares`，後續買賣不會改變該次應領股數。
- 官方更新只會修正預計資訊，不會覆蓋你已確認的 `actualPayDate` / `actualAmount`。
- 到預計發放日只顯示待確認；你確認後才同步建立投資股息與記帳收入。
- 為避免升級後製造歷史重複事件，第一次啟用只自動建立啟用日當天及之後的事件；既有歷史事件仍保留。
