...
function collectHomeReminders(){
  ...
  dueDividends.slice(0,2).forEach(e=>items.unshift({icon:'💰',title:`${e.symbol} 股息${e.status==='due'?'今天預計入帳':'待確認入帳'}`,meta:`${e.estimatedAmount==null?'金額待確認':`預估 ${money(e.estimatedAmount)}`} · 預計 ${e.expectedPayDate}`,tone:'income',action:()=>confirmDividendEvent(e.id)}));
  if(!dueDividends.length){const upcomingDividend=dividendUpcoming(30)[0];if(upcomingDividend)items.unshift({icon:'🗓️',title:`${upcomingDividend.symbol} 股息將入帳`,meta:`預計 ${upcomingDividend.expectedPayDate}${upcomingDividend.estimatedAmount==null?'':` · 約 ${money(upcomingDividend.estimatedAmount)}`}`,tone:'info',action:()=>openDividendEventEditor(upcomingDividend.id)});}
...
function setPage(page){ if(page==='analysis'){analysisYear=viewMonth.getFullYear();analysisMonth=viewMonth.getMonth();} for(const s of ['homeScreen','budgetScreen','investmentScreen','analysisScreen'])hide($(s)); if(page==='budget')show($('budgetScreen')); else if(page==='investment')show($('investmentScreen')); else if(page==='analysis')show($('analysisScreen')); else show($('homeScreen')); document.querySelectorAll('.nav-item[data-page]').forEach(b=>b.classList.toggle('active',b.dataset.page===page)); renderAll(); }
...
let securityDirectoryPromise=null,securityLookupTimer=null;
...
function guessSecurityType(symbol,name='',hint=''){
  ...
  if(/債.*ETF|ETF.*債|投等債|公司債|公債/.test(text))return 'bond-etf';
...
function normalizeSecurityMeta(meta,symbol){
  ...
  return {symbol:key,name:name||key,shortName:String(meta?.shortName||'').trim()||shortenSecurityName(name||key,key),securityType,market:meta?.market||'',updatedAt:meta?.updatedAt||new Date().toISOString()};
}
function cachedSecurityMeta(symbol){
  const key=investmentAssetKey(symbol); if(!key)return null;
  const cache=investmentSecurityCache(); return cache[key]?normalizeSecurityMeta(cache[key],key):(SECURITY_SEED[key]?normalizeSecurityMeta(SECURITY_SEED[key],key):null);
}
function pickField(obj,names){for(const n of names){if(obj&&obj[n]!=null&&String(obj[n]).trim())return String(obj[n]).trim();}return '';}
function parseSecurityRows(rows,{market='',sourceType=''}={}){
  const out=[]; for(const r of Array.isArray(rows)?rows:[]){
    const symbol=investmentAssetKey(pickField(r,['Code','證券代號','公司代號','股票代號','SecuritiesCompanyCode','SecuritiesCode','代號','基金代號'])); if(!symbol)continue;
    const name=pickField(r,['Name','證券名稱','證券簡稱','公司簡稱','公司名稱','股票名稱','CompanyName','SecuritiesCompanyName','基金簡稱','基金名稱','中文簡稱','中文名稱']); if(!name)continue;
    out.push(normalizeSecurityMeta({name,market,sourceType,securityType:sourceType==='fund'?'etf':undefined},symbol));
  } return out;
}
...
async function loadSecurityDirectory(){
  if(securityDirectoryPromise)return securityDirectoryPromise;
  securityDirectoryPromise=(async()=>{
    const map=new Map(Object.entries(SECURITY_SEED).map(([k,v])=>[k,normalizeSecurityMeta(v,k)]));
    const sources=[
      ['https://openapi.twse.com.tw/v1/exchangeReport/STOCK_DAY_AVG_ALL',{market:'TWSE'}],
      ['https://openapi.twse.com.tw/v1/opendata/t187ap47_L',{market:'TWSE',sourceType:'fund'}],
      ['https://openapi.twse.com.tw/v1/opendata/t187ap03_L',{market:'TWSE',sourceType:'stock'}],
      ['https://www.tpex.org.tw/openapi/v1/tpex_mainboard_quotes',{market:'TPEx',sourceType:'stock'}]
    ];
    const results=await Promise.allSettled(sources.map(([url])=>fetchJsonWithTimeout(url)));
    results.forEach((res,i)=>{if(res.status!=='fulfilled')return;const meta=sources[i][1];for(const x of parseSecurityRows(res.value,meta)){const prev=map.get(x.symbol);map.set(x.symbol,normalizeSecurityMeta({...prev,...x,securityType:(meta.sourceType==='fund'?guessSecurityType(x.symbol,x.name,'ETF'):x.securityType)},x.symbol));}});
    return map;
  })().catch(e=>{console.warn('security directory',e);return new Map(Object.entries(SECURITY_SEED).map(([k,v])=>[k,normalizeSecurityMeta(v,k)]));});
  return securityDirectoryPromise;
}
async function resolveSecurityMeta(symbol,{force=false}={}){
  const key=investmentAssetKey(symbol);if(!key)return null;
  const cache=investmentSecurityCache(),cached=cache[key]||SECURITY_SEED[key];
  if(cached&&!force){const normalized=normalizeSecurityMeta(cached,key);cache[key]=normalized;return normalized;}
  const directory=await loadSecurityDirectory(),found=directory.get(key)||normalizeSecurityMeta({name:key,securityType:guessSecurityType(key,'')},key); cache[key]=found; return found;
}
...
async function fetchIntradayQuotes(){
  const pf=investmentPortfolio(dateKey(new Date())),symbols=pf.active.map(p=>p.symbol); if(!symbols.length){toast('目前沒有持股');return;}
  toast('正在更新今日股價…',1200); let ok=0;
  for(const symbol of symbols){
    try{
      const meta=await resolveSecurityMeta(symbol),market=meta?.market==='TPEx'?'otc':'tse',url=`https://mis.twse.com.tw/stock/api/getStockInfo.jsp?ex_ch=${market}_${encodeURIComponent(symbol)}.tw&_=${Date.now()}`;
      const data=await fetchJsonWithTimeout(url,6000),r=Array.isArray(data?.msgArray)?data.msgArray[0]:null; if(!r)continue;
      const p=Number(r.z&&r.z!=='-'?r.z:r.y),d=String(r.d||''); if(!(p>0))continue; const marketDate=d.length===8?`${d.slice(0,4)}-${d.slice(4,6)}-${d.slice(6,8)}`:dateKey(new Date());
      upsertInvestmentQuote({symbol,name:meta?.name||'',price:p,date:marketDate,priceType:'intraday',source:'TWSE MIS',fetchedAt:new Date().toISOString()});ok++;
    }catch(e){console.warn('intraday',symbol,e);}
  }
  await persistState();renderInvestment();toast(ok?`已更新 ${ok} 檔今日股價`:'今日股價暫時無法取得',2400);
}
...
function investmentPortfolio(asOf=dateKey(new Date())){
  ...
  for(const t of investmentSortedLedger(asOf)){
    ...
    const key=t.symbol; if(!key)continue;
    if(!map.has(key))map.set(key,{symbol:key,name:t.name||key,shortName:t.shortName||shortenSecurityName(t.name||key,key),securityType:t.securityType||guessSecurityType(key,t.name),quantity:0,cost:0,realized:0,dividends:0,totalBuyCost:0,lastTradeDate:'',lastTradePrice:0,costIncomplete:false,incompleteCostQuantity:0});
    const p=map.get(key); if(t.name)p.name=t.name;if(t.shortName)p.shortName=t.shortName;if(t.securityType)p.securityType=t.securityType;
    if(t.kind==='initial'||t.kind==='buy'){
      const q=Math.max(0,Number(t.quantity)||0), pending=Boolean(t.costPending)||Number(t.price||0)<=0;
      p.quantity+=q;
      if(pending){
        p.costIncomplete=true;p.incompleteCostQuantity+=q;
      }else{
        const c=t.kind==='buy'?investmentTxnCash(t):ntd(q*Math.max(0,Number(t.price)||0));
        p.cost+=c;p.totalBuyCost+=c;
      }
      p.lastTradeDate=t.date;if(Number(t.price)>0)p.lastTradePrice=Number(t.price)||0;
    }else if(t.kind==='sell'){
      const q=Math.max(0,Number(t.quantity)||0);
      if(q>p.quantity+1e-8){p.issues=(p.issues||[]).concat({type:'oversell',id:t.id,date:t.date,requested:q,available:p.quantity});continue;}
      const proceeds=ntd(q*Math.max(0,Number(t.price)||0)-Math.max(0,Number(t.fee)||0)-Math.max(0,Number(t.tax)||0));
      if(p.costIncomplete){
        // 成本未補時仍正確扣股數，但禁止製造看似精準的實現損益/剩餘成本。
        p.quantity=Math.max(0,p.quantity-q);
        p.performanceIncomplete=true;
        p.incompleteCostQuantity=Math.min(p.quantity,p.incompleteCostQuantity);
      }else{
        const avg=p.quantity>0?p.cost/p.quantity:0, removed=ntd(avg*q);
        p.quantity=Math.max(0,p.quantity-q);p.cost=Math.max(0,p.cost-removed);p.realized+=proceeds-removed;
      }
      p.lastTradeDate=t.date;p.lastTradePrice=Number(t.price)||0;
    }else if(t.kind==='dividend')p.dividends+=ntd(Math.max(0,Number(t.amount)||0));
  }
  const positions=[...map.values()].map(p=>{
    const qp=investmentFallbackPrice(p.symbol,asOf), currentPrice=qp.price||p.lastTradePrice||0, marketValue=ntd(p.quantity*currentPrice);
    const incomplete=Boolean(p.costIncomplete||p.performanceIncomplete);
    const unrealized=incomplete?null:ntd(marketValue-p.cost), totalProfit=incomplete?null:ntd(p.realized+p.dividends+unrealized);
    return {...p,costIncomplete:incomplete,currentPrice,quoteDate:qp.date,priceSource:qp.source,marketValue,unrealized,totalProfit,roi:(!incomplete&&p.totalBuyCost>0)?totalProfit/p.totalBuyCost:null,avgCost:(!incomplete&&p.quantity>0)?p.cost/p.quantity:null};
  });
  const active=positions.filter(p=>p.quantity>0.0000001);
  const incompletePerformance=positions.some(p=>p.costIncomplete);
  const complete=positions.filter(p=>!p.costIncomplete);
  const totals=complete.reduce((a,p)=>({cost:a.cost+p.cost,marketValue:a.marketValue+p.marketValue,realized:a.realized+p.realized,dividends:a.dividends+p.dividends,totalBuyCost:a.totalBuyCost+p.totalBuyCost,unrealized:a.unrealized+(p.unrealized||0),totalProfit:a.totalProfit+(p.totalProfit||0)}),{cost:0,marketValue:0,realized:0,dividends:0,totalBuyCost:0,unrealized:0,totalProfit:0});
  totals.marketValue=positions.reduce((sum,p)=>sum+p.marketValue,0);
  totals.dividends=positions.reduce((sum,p)=>sum+p.dividends,0);
  totals.unresolvedContributions=unresolvedContributions;
  ...
  return {positions,active,totals,issues};
}
...
function investmentXirr(flows){
  const clean=flows.filter(f=>Number(f.amount)!==0&&f.date).sort((a,b)=>a.date.localeCompare(b.date));
  if(clean.length<2||!clean.some(f=>f.amount<0)||!clean.some(f=>f.amount>0))return null;
  const t0=parseDateKey(clean[0].date).getTime();
  const npv=r=>clean.reduce((s,f)=>s+f.amount/Math.pow(1+r,(parseDateKey(f.date).getTime()-t0)/86400000/365),0);
  let lo=-0.9999,hi=10,flo=npv(lo),fhi=npv(hi); let expand=0;
  while(flo*fhi>0&&expand<12){hi*=2;fhi=npv(hi);expand++;}
  if(!Number.isFinite(flo)||!Number.isFinite(fhi)||flo*fhi>0)return null;
  for(let i=0;i<120;i++){const mid=(lo+hi)/2,fm=npv(mid);if(Math.abs(fm)<1e-7)return mid;if(flo*fm<=0){hi=mid;fhi=fm;}else{lo=mid;flo=fm;}}
  return (lo+hi)/2;
}
function investmentYearReturn(year){
  const today=dateKey(new Date()), start=`${year}-01-01`, prev=`${year-1}-12-31`, yearEnd=`${year}-12-31`, end=yearEnd<today?yearEnd:today;
  if(start>end)return null;
  const flows=[]; const opening=investmentValueAsOf(prev); if(opening>0)flows.push({date:start,amount:-opening});
  ...
  for(const t of rows){
    ...
    if(t.kind==='initial'||t.kind==='buy'){if(!t.costPending&&Number(t.price)>0)flows.push({date:t.date,amount:-c});buys+=c;}
    else if(t.kind==='contribution'){buys+=c;unresolved+=c;}
    else if(t.kind==='sell'){flows.push({date:t.date,amount:c});sells+=c;}
    else if(t.kind==='dividend'){flows.push({date:t.date,amount:c});dividends+=c;}
  }
  const relevant=investmentSortedLedger(end);
  const hasCostPending=relevant.some(t=>['initial','buy'].includes(t.kind)&&(t.costPending||Number(t.price||0)<=0));
  const hasAnyUnresolved=relevant.some(t=>t.kind==='contribution')||hasCostPending;
  const closing=investmentValueAsOf(end); if(closing>0)flows.push({date:end,amount:closing});
  return {year,start,end,opening,closing,buys,sells,dividends,unresolved,rate:hasAnyUnresolved?null:investmentXirr(flows),hasUnresolved:hasAnyUnresolved,hasCostPending,hasData:rows.length>0||opening>0||closing>0};
}
function renderInvestment(){
  ...
  $('invCost').textContent=t.costIncomplete?'待補成本':investmentMoney(t.cost);
  $('invTotalProfit').textContent=t.costIncomplete?'待補成本':investmentMoney(t.totalProfit);
  $('invTotalReturn').textContent=t.costIncomplete?'待補資料':investmentPct(t.roi);
  $('invDividendTotal').textContent=investmentMoney(allRecordedDividendIncome());
  $('invRealizedTotal').textContent=t.costIncomplete?'待補成本':investmentMoney(t.realized);
  $('invUnrealizedTotal').textContent=t.costIncomplete?'待補成本':investmentMoney(t.unrealized);
  $('invTotalProfit').classList.toggle('negative',!t.costIncomplete&&t.totalProfit<0);
  $('invTotalReturn').classList.toggle('negative',!t.costIncomplete&&Number(t.roi)<0);
  $('invUnrealizedTotal').classList.toggle('negative',!t.costIncomplete&&t.unrealized<0);
  $('invRealizedTotal').classList.toggle('negative',!t.costIncomplete&&t.realized<0);
  ...
  if($('investmentIntradayValue')){
    if(!todayIntraday.length){$('investmentIntradayValue').textContent='抓今日股價';if($('investmentIntradayBasis'))$('investmentIntradayBasis').textContent='';}
    else{
      ...
      $('investmentIntradayValue').textContent=investmentMoney(value);
      if($('investmentIntradayBasis'))$('investmentIntradayBasis').textContent=`今日 ${todayIntraday.length}/${pf.active.length} 檔已更新`;
    }
  }
  ...
  $('investmentSyncPendingAmount').textContent=costPendingRows.length?`${investmentMoney(pendingAmount)} · ${costPendingRows.length} 筆成本待補`:investmentMoney(pendingAmount);
  ...
  if(pf.active.length&&allocationTotal>0){
    ...
    const slices=[];
    if(sorted.length<=6) slices.push(...sorted.map(p=>({symbol:p.symbol,name:p.shortName||p.symbol,value:p.marketValue})));
    else{
      slices.push(...sorted.slice(0,5).map(p=>({symbol:p.symbol,name:p.shortName||p.symbol,value:p.marketValue})));
    ...
    slices.forEach((item,i)=>{
      ...
      const row=document.createElement(item.symbol?'button':'div'); row.className='investment-allocation-legend-item'; if(item.symbol)row.type='button';
      row.innerHTML=`<i style="background:${color}"></i><span>${escapeHtml(item.name)}</span><strong>${(pct*100).toFixed(pct<.1?1:0)}%</strong>`;
      if(item.symbol)row.onclick=()=>openInvestmentSecurityDetail(item.symbol);allocationLegend.appendChild(row);
    });
  ...
  pf.active.sort((a,b)=>b.marketValue-a.marketValue).forEach(p=>{
    ...
    const profitClass=!p.costIncomplete&&p.totalProfit<0?'negative':'',costText=p.costIncomplete?'成本待補':investmentPrice(p.avgCost),profitText=p.costIncomplete?'成本待補':`${investmentMoney(p.totalProfit)} · ${investmentPct(p.roi)}`;
    row.innerHTML=`<div class="investment-symbol">${escapeHtml(p.symbol)}</div><div class="investment-holding-main"><strong>${escapeHtml(p.name||p.symbol)}</strong><span>${p.quantity.toLocaleString('zh-TW',{maximumFractionDigits:4})} 股 · 均價 ${costText}</span><small>現價 ${investmentPrice(p.currentPrice)}${p.quoteDate?` · ${escapeHtml(p.quoteDate)}${p.priceSource==='trade'?'（最近交易價）':''}`:' · 尚未設定現價'}</small></div><div class="investment-holding-value"><strong>${investmentMoney(p.marketValue)}</strong><span class="${profitClass}">${profitText}</span><em>›</em></div>`;
    row.onclick=()=>openInvestmentSecurityDetail(p.symbol); holdings.appendChild(row);
  });
  ...
  yearRows.forEach(({y,r})=>{
    ...
    const pendingText=r.hasUnresolved?` · ${r.hasCostPending?'含成本待補':'待補成交 '+investmentMoney(r.unresolved||0)}`:'';
    el.innerHTML=`<div class="investment-year-label"><strong>${y}${y===currentYear?' · 今年':''}</strong><span>投入 ${investmentMoney(r.buys)} · 股息 ${investmentMoney(r.dividends)}${pendingText}</span></div><div class="investment-year-rate ${negative?'negative':''}">${r.hasUnresolved?'待補資料':investmentPct(r.rate)}</div><div class="investment-year-track"><div class="investment-year-fill ${negative?'negative':''}" style="width:${width.toFixed(1)}%"></div></div>`;
  ...
  const rows=investmentLedger.filter(t=>!t.voided).slice().sort((a,b)=>String(b.date).localeCompare(String(a.date))||String(b.createdAt||'').localeCompare(String(a.createdAt||''))).slice(0,30);
  ...
  rows.forEach(t=>{
    ...
    const flowClass=['buy','initial','contribution'].includes(t.kind)?'out':'in';
    const displaySymbol=t.symbol||t.title||'未指定標的';
    const detail=t.kind==='dividend'
      ?escapeHtml(t.name||'股息入帳')
      :t.kind==='contribution'
        ?`${escapeHtml(t.name||t.title||'記帳同步')} · 待補股數／成交價`
        :t.costPending
          ?`${escapeHtml(t.name||'')} · ${Number(t.quantity||0).toLocaleString('zh-TW',{maximumFractionDigits:4})} 股 · 成本待補`
          :`${escapeHtml(t.name||'')} · ${Number(t.quantity||0).toLocaleString('zh-TW',{maximumFractionDigits:4})} 股 @ ${investmentPrice(t.price)}`;
    const amountText=t.costPending?'待補成本':`${flowClass==='out'?'-':'+'}${investmentMoney(amt)}`;
    row.innerHTML=`<div class="investment-ledger-icon kind-${escapeHtml(t.kind)}">${investmentKindIcon(t.kind)}</div><div class="investment-ledger-main"><strong>${escapeHtml(displaySymbol)} <span class="investment-kind-chip">${investmentKindLabel(t.kind)}</span>${t.costPending?' <span class="investment-kind-chip">成本待補</span>':''}</strong><span>${detail}</span><small>${escapeHtml(t.date)}${t.datePending?' · 日期待補':''}${t.note?` · ${escapeHtml(t.note)}`:''}</small></div><div class="investment-ledger-amount ${flowClass}">${amountText}<em>›</em></div>`;
...
async function importBackupFile(file){ try{const data=JSON.parse(await file.text());if(data.app!=='little-days-bookkeeping'||!Array.isArray(data.txns))throw new Error('bad');const mergeOnly=Boolean(data.mergeOnly);if(!confirm(`匯入 ${data.txns.length} 筆紀錄？目前資料會以 ID 合併。${mergeOnly?'\n這是私人一次性資料檔，不會覆蓋其他設定。':''}`))return;const map=new Map(txns.map(t=>[t.id,t]));data.txns.forEach(t=>map.set(t.id||uid(),t));txns=[...map.values()];if(mergeOnly){if(Array.isArray(data.recurring)){const rmap=new Map(recurring.map(r=>[r.id,r]));data.recurring.forEach(r=>rmap.set(r.id||uid(),r));recurring=[...rmap.values()];}if(Array.isArray(data.investmentLedger)){const imap=new Map(investmentLedger.map(r=>[r.id,r]));data.investmentLedger.forEach(r=>imap.set(r.id||uid(),r));investmentLedger=[...imap.values()];}if(Array.isArray(data.investmentQuotes)){const qmap=new Map(investmentQuotes.map(r=>[r.id,r]));data.investmentQuotes.forEach(r=>qmap.set(r.id||uid(),r));investmentQuotes=[...qmap.values()];}if(Array.isArray(data.dividendEvents)){const dmap=new Map(dividendEvents.map(r=>[r.id,r]));data.dividendEvents.forEach(r=>dmap.set(r.id||uid(),r));dividendEvents=[...dmap.values()];}if(Array.isArray(data.annualIncomeSummaries)){const amap=new Map(annualIncomeSummaries.map(r=>[Number(r.year),r]));data.annualIncomeSummaries.forEach(r=>amap.set(Number(r.year),r));annualIncomeSummaries=[...amap.values()];}settings={...settings,lastRestoreAt:new Date().toISOString()};}else{budgets={...budgets,...(data.budgets||{})};if(Array.isArray(data.categories)&&data.categories.length)categories=data.categories;if(Array.isArray(data.quickTemplates)&&data.quickTemplates.length)quickTemplates=data.quickTemplates;if(Array.isArray(data.recurring))recurring=data.recurring;if(Array.isArray(data.investmentLedger))investmentLedger=data.investmentLedger;if(Array.isArray(data.investmentQuotes))investmentQuotes=data.investmentQuotes;if(Array.isArray(data.dividendEvents))dividendEvents=data.dividendEvents;if(Array.isArray(data.annualIncomeSummaries))annualIncomeSummaries=data.annualIncomeSummaries;settings={...settings,...(data.settings||{}),lastRestoreAt:new Date().toISOString()};}normalizeData();migrateHistoricalCategorySummaries();normalizeData();await persistState();renderAll();renderBackupStatus();toast(mergeOnly?'私人資料合併完成':'備份還原完成');}catch{toast('這不是有效的記帳備份檔',2600);} }
