...
function profile(x,y,{left=68,employee=250,applicant=426}={}){
  txt(x,`部門代號：${PROFILE.department}`,left,y,9);
  txt(x,`員工編號：${PROFILE.employeeNo}`,employee,y,9);
...
function generalFrame(x,slot){
  ...
  profile(x,t+137,{left:68,employee:250,applicant:GENERAL_X[6]});
...
function noFrame(x,slot){
  ...
  txt(x,'總        計',205,t+125.5,10,'center');signatures(x,t,'no',slot);profile(x,t+149,{left:70,employee:250,applicant:NO_X[4]});
